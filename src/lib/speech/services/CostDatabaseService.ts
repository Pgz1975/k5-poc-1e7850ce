...
export class CostDatabaseService {
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          estimated_cost: this.currentSession.estimatedCost
        })
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          actual_cost: this.currentSession.estimatedCost
        })
        .eq('id', this.currentSession.id);
  ...
  async getStudentUsageStats(studentId: string, days: number = 30): Promise<any> {
    ...
    try {
      ...
      const statsByModel = data.reduce((acc: any, session: any) => {
        const model = session.model_type;
        if (!acc[model]) {
          acc[model] = {
            sessions: 0,
            totalCost: 0,
        ...
        acc[model].sessions++;
        acc[model].totalCost += session.actual_cost || 0;
        acc[model].totalDuration += session.duration_seconds || 0;
      ...
      return {
        ...
        totalCost: data.reduce((sum, s) => sum + (s.actual_cost || 0), 0),
  ...
  async getOrganizationStats(days: number = 30): Promise<any> {
    ...
    try {
      ...
      const { data: costs, error: costsError } = await supabase
        .from('model_usage_costs')
        .select('model_type, actual_cost, duration_seconds')
        .gte('created_at', startDate.toISOString());
      ...
      return {
        totalCost: costs.reduce((sum, c) => sum + (c.actual_cost || 0), 0),
        totalSessions: costs.length,
        totalSwitches: switches.length,
        averageCostPerSession: costs.length > 0
          ? costs.reduce((sum, c) => sum + (c.actual_cost || 0), 0) / costs.length
          : 0,
  ...
  private calculateModelDistribution(costs: any[]): Record<string, number> {
    ...
    costs.forEach(cost => {
      distribution[cost.model_type] = (distribution[cost.model_type] || 0) + 1;
    });
