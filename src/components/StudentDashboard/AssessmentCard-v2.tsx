import { Star, ClipboardCheck, Medal, Trophy, Play, RotateCcw, Clock } from "lucide-react";
import { useLanguage } from "@/contexts/LanguageContext";
import { useNavigate } from "react-router-dom";
import { cn } from "@/lib/utils";

interface AssessmentCardV2Props {
  id: string;
  title: string;
...
export function AssessmentCardV2({
  id,
  title,
  description,
  estimatedDuration,
  ...
  const getMedalIcon = (stars: number) => {
    if (stars === 5) return Trophy;
    if (stars >= 3) return Medal;
  ...
  const stars = completionData ? getStars(completionData.score) : 0;
  const MedalIcon = getMedalIcon(stars);
    ...
    >
      ...
      {estimatedDuration && (
        <div className="flex items-center gap-2 text-sm font-bold text-gray-600 mb-4">
          ...
          <span>{estimatedDuration} {t("minutos", "minutes")}</span>
        </div>
      )}
